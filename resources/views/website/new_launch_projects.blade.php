            ...
            <div class="section_title text-start text-md-start">
                <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active fs-tiny capitalize" aria-current="page">{{$pagename}}
                        </li>
                    </ol>
                </nav>
                <h3 class="capitalize">{{$pagename}}</h3>
            </div>
                ...
                <div class="bg-white mb-4">
                    <div class="d-md-flex">
                        <div class="img_box image_swiper_box">
                            <div id="carouselItem_{{$property->property->id}}" class="carousel slide carousel-fade">
                                <a href="{{ route('property', ['propertySlug' => $property->property->slug]) }}">
                                    <div class="carousel-inner">
                                        <div class="carousel-item active">
                                            @if($property->property->images && count($property->property->images) > 0)
                                        ...
                                        @if($property->property->images && count($property->property->images) > 1)
                                        <div class="carousel-item">
                                            <img src="{{ asset('images/property_images/'.$property->property->images[1]->name)}}"
                                                class="d-block w-100" alt="{{$property->property->images[1]->aux1}}">
                                        </div>
                                        @endif
                                        @if($property->property->images && count($property->property->images) > 2)
                                        <div class="carousel-item">
                                            <img src="{{ asset('images/property_images/'.$property->property->images[2]->name)}}"
                                                class="d-block w-100" alt="{{$property->property->images[2]->aux1}}">
                                        </div>
                                        @endif
                                    </div>
                                </a>
                                <button class="carousel-control-prev" type="button"
                                    data-bs-target="#carouselItem_{{$property->property->id}}" data-bs-slide="prev">
                                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                                    <span class="visually-hidden">Previous</span>
                                </button>
                                <button class="carousel-control-next" type="button"
                                    data-bs-target="#carouselItem_{{$property->property->id}}" data-bs-slide="next">
                            ...
                            <div class="action_buttons">
                                ...
                                <div class="d-inline-block">
                                    <a href="#" data-property-id="{{ $property->property->id }}">
                                        @auth
                                        @if(in_array($property->property->id, $userPropertyseenIds->toArray()))
                                ...
                                <div class="d-inline-block">
                                    <a href="#" data-property-id="{{ $property->property->id }}">
                                        @auth
                                        @if(in_array($property->property->id, $userPropertyEnquiryIds->toArray()))
                                ...
                                <div class="d-inline-block">
                                    <a href="#" class="favorite-button" data-property-id="{{ $property->id }}">
                                        <div class="box_default me-1" title="Favourite">
                                            @auth
                                            <i
                                                class="fs- @if(in_array($property->property->id, $userFavoritePropertyIds->toArray())) bi bi-heart-fill @else bi bi-heart @endif favorite-icon fs-10"></i>
                                            @else
                                            <i
                                                class="fs- @if(in_array($property->property->id, $userFavoritePropertyIds)) bi bi-heart-fill @else bi bi-heart @endif favorite-icon fs-10"></i>
                        ...
                        <div class="content_box flex-grow-1 px-3 py-2">
                            <div class="d-flex justify-content-end ">
                                <h6 id="top_properties_price" class="mb-2">₹{{$property->property->price}}</h6>
                            </div>
                            <div class="overflow-hidden">
                                <a href="{{ route('property', ['propertySlug' => $property->property->slug]) }}"
                                    class="link_default">
                                    <h6 class="item_title mb-2">{{$property->property->title}}</h6>
                                </a>
                                <p class="item_title mb-2">{{$property->property->address}}</p>
                                <div class="mb-2 d-flex">
                                    <div class="fs-tiny me-3"> <i class="bi bi-rulers"></i> {{$property->property->area}}</div>
                                    <p class="fw-400 uppercase mb-2">{{$property->property->property_type->name}}</p>
                                </div>
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="d-flex">
                                        ...
                                        <div class="fs-tiny">
                                            <i
                                                class="bi bi-paperclip"></i>{{ $property->property->created_at->diffInMonths(now()) }}
                                            months ago
                                        </div>
                                    </div>
                                    <a href="{{ route('property', ['propertySlug' => $property->property->slug ]) }}"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        previewButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                modalTitle.textContent = propertyData.property.title;
                modalAddress.textContent = propertyData.property.address;
    
                propertyData.property.images.forEach((image, index) => {
                ...
                modalDescription.textContent = stripTags(propertyData.property.description);
                modalPrice.textContent = "₹ " + propertyData.property.price;
                modalArea.textContent = "₹ " + (propertyData.property.price / propertyData.property.area).toFixed(2) + "/sqft";
            });
        });
    });
</script>
