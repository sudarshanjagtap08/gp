...
// const itemCount = 6; // Initial item count
...
const paginationNumbers = document.getElementById("pagination-numbers-categoryList");
const paginatedList = document.getElementById("paginated-list-categoryList");
const listItems = paginatedList.querySelectorAll(".categoryItem");
const nextButton = document.getElementById("next-button-categoryList");
const prevButton = document.getElementById("prev-button-categoryList");
...
function openReviewTab(){
    // let btn_box = $('#pills-tabListingAuthor');
    let btn_box = document.querySelector('#pills-tabListingAuthor')
    let pill_items = btn_box.querySelectorAll('button');

    let tabListingAuthor = document.querySelector('#pills-tabContentAuthor')
    let tab_pane = tabListingAuthor.querySelectorAll('.tab-pane');
    console.log(tab_pane)

    removeActive(pill_items,tab_pane);
    addActive(pill_items,tab_pane);
}
function removeActive(pill_items,tab_pane){
    pill_items.forEach(function(e){
       e.classList.remove('active');
   });
   tab_pane.forEach(function(e){
       e.classList.remove('active');
       e.classList.remove('show');
   });
}
function addActive(pill_items,tab_pane){
    for(let i=0;i<2;i++){
        if(i==1){ 
            pill_items[1].classList.add('active');
         }
    }
    for(let i=0;i<2;i++){
        if(i==1){ 
            tab_pane[1].classList.add('show');
...
//     let lastScrollTop = 0;
//     const scrollDiv = $(".add_sticky_class");
//     alert()
...
//         if (st > lastScrollTop) {
//             // Scrolling down
//             scrollDiv.removeClass("added-class");
//         } else {
//             // Scrolling up
//             scrollDiv.addClass("added-class");
//         }
//         lastScrollTop = st;
//     });
// });
